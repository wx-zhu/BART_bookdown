...
author: "Jingyi Guan, Alayana Joshson, Wenxuan Zhu"
date: "`r Sys.Date()`"
...
In this project, we study a Bayesian approach called BART (Bayesian Additive Regression Trees) which uses a sum of trees to model or approximate $f(x) = E(Y | x)$. The essential idea is to elaborate the sum-of-trees model by imposing a prior that regularizes the fit by keeping the individual tree effects small. In effect, the $g_j$‘s become a dimensionally adaptive random basis of “weak learners.” By weakening the $g_j$ effects, BART ends up with a sum of trees, each of which explains a small and different portion of f.
