...
## `dbarts` package
```{r}
bartFit <- bart(x.train = as.matrix(df_train[,5:7]), y.train = as.numeric(unlist(df_train[,3])), keeptrees = TRUE)

trees <- extract(bartFit, "trees")


bartFit$fit$plotTree(chainNum = 1, sampleNum = 3, treeNum = 3)
bartFit$fit$plotTree(chainNum = 1, sampleNum = 3, treeNum = 140)
bartFit$fit$plotTree(chainNum = 1, sampleNum = 3, treeNum = 152)
```
