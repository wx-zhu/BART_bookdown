...
2.  The distribution on the splitting variable assignments at each
    interior node, where $p$ denotes the splitting variable
...
3.  **Splitting rule**: For the chosen variable, the specific way it splits (i.e., the rule it uses to divide the data) is also chosen randomly. This is controlled by setting the distribution on the splitting rule assignment in each interior node, conditional on the splitting variable
