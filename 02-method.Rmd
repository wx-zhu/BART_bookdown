...
that has $b$ terminal nodes, let $M_j = \{\mu_{1j}, \mu_{2j}, \mu_{3j}, ..., \mu_{bj}\}$
...
Priors over $(T_1, M_1), (T_2, M_2), (T_3, M_3), ... (T_m, M_m)$ and the prior over $\sigma$. For all those priors, we have their joint probability function as: 
...
& = [\prod^{m}_{j=1} P(\mu_{1j}|T_{j})P(\mu_{2j}|T_{j})...P(\mu_{bj}|T_{j})]P(\sigma)\;\;  \text{independence of parameters associated with nodes in a specific tree}\\ 
& = [\prod^{m}_{j=1} \prod^{b_j}_{i=1} P(\mu_{ij}|T_j)P(T_j)]P(\sigma) \end{align*} $$
Thus, after those calculation, we figured that there are only $4$
priors that we need to think about, which are $T_j, \mu_{ij}|T_j$, $\sigma$ and $m$. We will talk about them one by one in the next section.
...
1.  **Depth-based stopping**: The deeper a node in the tree, the less
    ...
    manipulating the probability that a node at depth $d$ (= 0, 1, 2, ...) is nonterminal. Specifically, parameters $\alpha$ and $\beta$ are
...
For quantitative variable $l_q$, 
$$\text{splitting rule | variable } l_q \sim Unif(min(l_q), max(l_q))$$
For categorical variable $l_c$, each category has the same chance of being chosen.

All these prior settings about $T_j$ help ensure that the trees remain
**small** and manageable, which is important when they are combined to form the sum of trees model.
...
predictive performance significantly improves from $m = 1$ to around 200 trees. However, excessively large $m$ values can lead to a gradual
...
$(T_1, M_1), ..., (T_m, M_m)$ and $\sigma$, the posterior distribution of
...
parameter in $(T_1, M_1), ...(T_m, M_m)$ and $\sigma$ using the most current
knowledge of the other parameters, which is just the idea of Gibbs
sampler.

Specifically, let $T_{(j)}$ represent all trees except $T_j$, and let $M_{(j)}$
represent all terminal nodes parameters except $M_j$. In this Gibbs
sampler, we loop from $j = 1$ to $j = m$ to draw $(T_j, M_j)$ given
...
tree through 4 modifications: growing a terminal node, pruning a
