...
$$f(x) \approx h(x) ≡ \sum_{j=1}^m g_j(x) $$
,where each $g_j$ denotes the mapping function associated with the $j_{th}$ tree.
...
priors that we need to think about, which are $\mu_{ij}|T_j, T_j$, and $\sigma$. We will talk about them one by one in the next section.
